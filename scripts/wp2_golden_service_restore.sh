...
bar 10 'Stopping UE and clearing profile session/tunnel'; echo
ssh_do "$UE_HOST" "tmux kill-session -t ue >/dev/null 2>&1 || true; tmux kill-session -t srs-ue >/dev/null 2>&1 || true; sudo pkill -TERM -x srsue 2>/dev/null || true; for i in \$(seq 1 20); do pgrep -x srsue >/dev/null || break; sleep 0.5; done; sudo pkill -KILL -x srsue 2>/dev/null || true; ip link show tun_srsue >/dev/null 2>&1 && sudo ip link delete tun_srsue 2>/dev/null || true; ! tmux has-session -t ue >/dev/null 2>&1 && ! tmux has-session -t srs-ue >/dev/null 2>&1 && ! pgrep -x srsue >/dev/null"
printf 'T_UE_STOPPED=%s\n' "$(utc)"

bar 25 'Stopping EPC/eNB and clearing profile session'; echo
ssh_do "$CORE_HOST" "tmux kill-session -t enb >/dev/null 2>&1 || true; tmux kill-session -t srs-epc >/dev/null 2>&1 || true; tmux kill-session -t srs-enb >/dev/null 2>&1 || true; sudo pkill -TERM -x srsenb 2>/dev/null || true; sudo pkill -TERM -x srsepc 2>/dev/null || true; sleep 2; sudo pkill -KILL -x srsenb 2>/dev/null || true; sudo pkill -KILL -x srsepc 2>/dev/null || true; ! tmux has-session -t enb >/dev/null 2>&1 && ! tmux has-session -t srs-epc >/dev/null 2>&1 && ! tmux has-session -t srs-enb >/dev/null 2>&1 && ! pgrep -x srsenb >/dev/null && ! pgrep -x srsepc >/dev/null"
