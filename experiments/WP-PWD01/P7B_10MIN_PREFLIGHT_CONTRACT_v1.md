...
## Maximum scope
The capsule MAY read Portal identity/manifest, resolve node logins, SSH to already-created nodes, read OS/tool/runtime versions, syntax-check intended sources with the exact executing interpreter, verify absolute path rules, verify preservation tooling, inspect already-preserved attenuator command evidence/fixtures, and emit a bounded report.

The capsule MUST NOT create/extend/terminate a reservation, invoke `tmcc attenuator` as a presumed readback probe, change attenuation, start B1/W1/B2, start/restart broker/gateway/generator/receiver, publish scientific telemetry, change scored authorization, or delete evidence. A live attenuator CLI probe is allowed only if a separately documented read-only interface exists; the observed POWDER interface did not establish such a readback operation.
...
4. `EXTERNAL_INTERFACE_GATE`: use preserved attenuator command fixtures plus documented interface semantics; do not invent a readback operation. Distinguish command failure, parser failure and semantic mismatch. Portal transport errors are observations, not experiment-state verdicts.
...
8. `DEPENDENCY_GATE`: every role uses only required tools. Any download/install occurs before RF mutation, is exact-version/hash controlled, and fails closed on repository/signature errors.
9. `PORTAL_SEMANTICS_GATE`: generic `GET_ERROR` means unknown control-plane state; it cannot mean NOT_READY, NOT_FOUND, successful teardown, or scientific failure.
